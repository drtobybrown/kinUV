...
def run_target(config_path, s3_root, covariance_metrics, output):
    ...
    cube_yxv = cube_yxv[:, :, n_guard:-n_guard]
    native_radio_lsrk = topo_radio_to_lsrk_radio(
        data.vel_native[n_guard:-n_guard], correction
    )
    model_header = _model_header(grid, native_radio_lsrk, data_header)
    model_fits_order = np.flip(np.moveaxis(cube_yxv, 2, 0), axis=2)
    pb_frequency_hz = float(np.median(data.freqs_native))
    model_k, _, dv_data = match_model_to_imaging(
        model_fits_order,
        model_header,
        data_header,
        undo_pb=True,
        nu_hz=pb_frequency_hz,
    )
    ...
    record = {
        ...
        "primary_beam": {
            "forward_frequency_hz": pb_frequency_hz,
            "diagnostic_inverse_frequency_hz": pb_frequency_hz,
            "accounting": "same median native observed frequency in forward and inverse",
        },
        "spectral_response": {
            "model": "native Hann [0.25, 0.5, 0.25], discard guards, overlap-average to official cube edges",
            "data_lineage": "calibrated MS channels already Hanning-correlated; official cube made by tclean with smoothfactor=1.0",
            "additional_image_domain_hann": False,
        },
        "noise": {
...
def run_target_replay(config_path, s3_root, covariance_metrics, covariance_path, output):
    """Re-score every frozen S3 ancestor under repaired diagnostic accounting."""
    ...
    mask = np.asarray(fits.getdata(mask_path), dtype=np.float64) > 0.5
    error_moment0 = np.squeeze(np.asarray(fits.getdata(error_path), dtype=np.float64))
    dv_header = float(abs(data_header["CDELT3"]))
    if str(data_header.get("CUNIT3", "km/s")).lower().replace(" ", "") in {
        "m/s",
        "ms-1",
    }:
        dv_header /= 1.0e3
    sigma_channel, noise_pixels = channel_noise_from_integrated_error(
        error_moment0, mask, dv_header
    )
    kinms_result = json.loads(Path(config["kinms"]["result"]).read_text(encoding="utf-8"))
    correction = float(config["spectral_frame"]["frequency_correction_equivalent_kms"])
    pb_frequency_hz = float(np.median(data.freqs_native))
    n_guard = int(data.n_guard)
    velocity_core = data.vel_native[n_guard:-n_guard]
    native_radio_lsrk = topo_radio_to_lsrk_radio(velocity_core, correction)
    model_header = _model_header(grid, native_radio_lsrk, data_header)
    common_geometry = {
        "pa_deg": float(config["geometry"]["pa_seed_deg"]),
        "inclination_deg": float(config["geometry"]["inclination_deg"]),
        "vsys_kms": float(config["geometry"]["vsys_optical_seed_kms"]),
        "dx_arcsec": float(config["stage_a"]["parameter_seed"]["dx_arcsec"]),
        "dy_arcsec": float(config["stage_a"]["parameter_seed"]["dy_arcsec"]),
        "source": "frozen target configuration, independent of S3 candidate",
    }
    destination = Path(output) / target_id
    destination.mkdir(parents=True, exist_ok=True)
    candidate_records = []
    ...
    for fit in s3["fits"]:
        ...
        if use_emissivity:
            emissivity_weights = np.asarray(
                parameters["emissivity_weights"], dtype=np.float64
            )
            template = np.sum(
                emissivity_weights[:, None, None] * basis.images, axis=0
            )
        else:
            template = base_template
        inclination_rad = np.radians(float(parameters["inclination_deg"]))
        uses_rings = candidate == "supported_rings" or (
            candidate == "two_zone_dispersion"
            and dispersion_parent == "supported_rings"
        )
        velocity_profile = (
            _ring_profile(knot_radii, parameters["u_knots_kms"], inclination_rad)
        ...
        cube_yxv = np.asarray(hann_native(cube_yxv, axis=2))[:, :, n_guard:-n_guard]
        model_fits_order = np.flip(np.moveaxis(cube_yxv, 2, 0), axis=2)
        model_k, _, dv_data = match_model_to_imaging(
            model_fits_order,
            model_header,
        ...
        candidate_dir = destination / "candidates" / candidate
        candidate_dir.mkdir(parents=True, exist_ok=True)
        kinuv_cube_path = candidate_dir / "kinuv_model_k.fits"
        output_header = data_header.copy()
        output_header["BUNIT"] = "K"
        output_header["ORIGIN"] = "kinUV S4 repaired frozen-parameter replay"
        fits.PrimaryHDU(model_k.astype(np.float32), output_header).writeto(
            kinuv_cube_path, overwrite=True
        )
        benchmark = write_cube_benchmark(
            target_id=f"{target_id} {candidate}",
            data_cube=data_cube_path,
            mask_cube=mask_path,
            kinuv_cube=kinuv_cube_path,
            kinms_cube=Path(config["kinms"]["cube"]),
            output_dir=candidate_dir / "benchmark",
            pa_deg=common_geometry["pa_deg"],
            inclination_deg=common_geometry["inclination_deg"],
            vsys_kms=common_geometry["vsys_kms"],
            dx_arcsec=common_geometry["dx_arcsec"],
            dy_arcsec=common_geometry["dy_arcsec"],
        )
        kinms_k = np.asarray(
            fits.getdata(candidate_dir / "benchmark" / "kinms_model_k.fits"),
            dtype=np.float64,
        )
        kinuv_chi2 = common_reduced_chi2(
            observed,
            ...
            mask,
            sigma_channel,
            _active_parameter_count(fit),
        )
        if kinms_chi2 is None:
            kinms_chi2 = common_reduced_chi2(
                observed,
                kinms_k,
                mask,
        ...
        candidate_record = {
            "candidate": candidate,
            "selected_in_s3": candidate == preferred_name,
            "frozen_parameters": parameters,
            "training_chi2": fit["chi2"],
            "active_parameter_count": _active_parameter_count(fit),
            "cube_metrics": benchmark["metrics"],
            "diagnostic_voxel_chi2": {
                "kinuv": kinuv_chi2,
                "kinms": kinms_chi2,
                "ratio_kinuv_over_kinms": kinuv_chi2["reduced_chi2"]
                / kinms_chi2["reduced_chi2"],
                "promotion_role": "supporting in-sample diagnostic only",
            },
            "diagnostic_moment1_recovery": recovery,
            "products": str(candidate_dir.relative_to(output)),
        }
        ...
        print(
            json.dumps(
                {
                    "candidate": candidate,
                    "moment1_rmse_ratio": recovery["ratio_kinuv_over_kinms"],
                    "target": target_id,
                    "voxel_chi2_ratio": candidate_record["diagnostic_voxel_chi2"][
    ...
    structured = _structured_diagnostics(config, covariance_metrics)
    record = {
        "schema_version": "kinuv-s4-repair-replay-v1",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "target_id": target_id,
        "step": "P0 repaired frozen-parameter replay",
        "promotion_eligible": False,
        "promotion_blockers": [
            "known-truth paired recovery has not been run under the prospective contract",
            "training-only KinMS and kinUV fits have not been scored on untouched line visibilities",
        ],
        "preferred_s3_candidate": preferred_name,
        "common_diagnostic_geometry": common_geometry,
        "inputs": {
            "config": {"path": str(config_path.resolve()), "sha256": sha256(config_path)},
            "s3": {"path": str(target_s3.resolve()), "sha256": sha256(target_s3)},
            "covariance": {
                "path": str(Path(covariance_path).resolve()),
                "sha256": sha256(Path(covariance_path)),
            },
            "data_cube": {"path": str(data_cube_path), "sha256": sha256(data_cube_path)},
            "mask_cube": {"path": str(mask_path), "sha256": sha256(mask_path)},
            "integrated_error": {"path": str(error_path), "sha256": sha256(error_path)},
        ...
        "load": load_metadata,
        "accounting_repairs": {
            "sky_coordinates": "astropy celestial WCS spherical offsets; positive east/north; no extra CDELT1 reflection",
            "profile_support": "one data-defined moment-0 support and common frozen geometry for all cubes and candidates",
            "primary_beam_frequency_hz": pb_frequency_hz,
            "primary_beam": "same median native observed frequency in forward and inverse",
            "spectral_response": "one native Hann on model with discarded guards, then channel-edge overlap onto official tclean cube",
            "official_cube_history": "tclean; smoothfactor=1.0; calibrated MS channels already Hanning-correlated",
        },
        "frame": {
            "likelihood": "native TOPO radio frequency",
            "diagnostic": "LSRK optical velocity",
        ...
        "structured_line_free_residuals": structured,
        "mechanical_replay_complete": len(candidate_records) == len(s3["fits"]),
    }
    write_json_atomic(destination / "replay.json", record)
    return record
...
def main():
    ...
    parser.add_argument(
        "--replay-all",
        action="store_true",
        help="repair-accounting replay of every frozen S3 candidate; never promotes S4",
    )
    ...
    summary = {
        "schema_version": "kinuv-s4-repair-replay-summary-v1"
        if args.replay_all
        else "kinuv-s4-real-summary-v1",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "git": state,
        "pi_gate": (
            {
                "known_truth_projected_velocity_rmse_ratio_max": 0.90,
                "real_held_out_visibility_improvement_required": True,
                "status": "not evaluated by mechanical replay",
        ...
        "targets": [
            {
                ...
                **(
                    {
                        "mechanical_replay_complete": row[
                            "mechanical_replay_complete"
                        ],
                        "promotion_eligible": row["promotion_eligible"],
                        "preferred_s3_candidate": row["preferred_s3_candidate"],
                    }
                    if args.replay_all
                    else {
                        "accepted": row["accepted"],
                        "gates": row["gates"],
                        "projected_velocity_recovery": row[
                            "projected_velocity_recovery"
                        ],
        ...
        "accepted": False
        if args.replay_all
        else all(row["accepted"] for row in records),
        "mechanical_replay_complete": all(
            row.get("mechanical_replay_complete", False) for row in records
        )
        if args.replay_all
        else None,
    }
    ...
    print(
        json.dumps(
            {
                "stage": "S4-P0-replay" if args.replay_all else "S4",
                "accepted": summary["accepted"],
                "mechanical_replay_complete": summary.get(
                    "mechanical_replay_complete"
                ),
            },
            sort_keys=True,
    ...
    if not args.replay_all and not summary["accepted"]:
