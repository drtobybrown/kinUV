...
Not a DEC. Not a 400-galaxy runner. [`DEC-066-TARGET`](../decisions/DEC-066-TARGET.md) still locks **code** to KGAS066 until MAP beats \(V=0\) (passed) **and** injected vsys/PA/flux recover on 066 uv (Hann+bin mock; see methodology propose). [`DEC-HIER-SELFUNC`](../decisions/DEC-HIER-SELFUNC.md) stays deferred.
...
At ~0.5 s/eval CPU: Stage A MAP (~10³ evals) is minutes per galaxy → 400 MAP ~ hours–a day on a small CPU pool. NUTS at \(10^5\) evals is ~12–17 h **per galaxy** on CPU → not a survey product until eval/s rises (~JAX T2, then CANFAR GPU as the same model).
