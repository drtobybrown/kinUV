...
This is the historical KGAS066 kernel sequence, not a population runner. The current accepted bundle is **`kinuv-KGAS066-4c1bc4-milestone1`**. The accepted review is summarized in [`../PRODUCTION_RECORD.md`](../PRODUCTION_RECORD.md). Leftover-vs-velocity is a G0 flag; flags live in `kinuv.diagnostics`; G1 is CPU JAX only.

## What is already true

- Vis Stage A beats V=0 (`Delta_chi2` = +35553). S1 recovered a sub-beam inject on Hann+bin; the CLEANed cube did not.
- Receding 066 CPU NUTS exists (`sampler: nuts`, session `sd3ckpf2`). Approaching PA 25.2 is this card. S2 was `laplace_mh`. Laplace 68/95 intervals **failed** SBC. Do not quote S2 or uncalibrated 066 16/50/84.
...
| G3 | NumPyro NUTS. Receding init is official MAP PA **199.73°** (not L-BFGS seed 205.2°); approaching 25.2°. 4 chains. Label `sampler: nuts` only after autodiff. **Landed 2026-08-31 receding:** session `sd3ckpf2`, mixing pass, `chi2` at NUTS mean 167487 vs MAP 168676. Approaching PA 25.2 is this leftover card (`nuts-pa25`). No GPU. | no |
