...
This is the historical KGAS066 kernel sequence, not a population runner. The current accepted model is **`results/production/KGAS066/best_model/`**. The accepted review is summarized in [`../PRODUCTION_RECORD.md`](../PRODUCTION_RECORD.md). Leftover-vs-velocity is a G0 flag; flags live in `kinuv.diagnostics`; G1 is CPU JAX only.
