...
3. `docs/architecture/STATUS.md` — mailbox (`next_role`, `pending`, `code_freeze`).
4. `docs/reviews/` — handshake log; not physics.
...
A worker that needs a new decision **stops**. It does not create a `DEC-*` id. The user adds stubs. Two-agent edits follow `DEC-066-AGENTS`: no `accepted` without a review ACK; no rubber-stamp.
