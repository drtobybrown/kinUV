...
3. Compute on `/scratch/kinuv-$USER/<session>` (TMP, JAX cache). Durable products — manifests, **job-owned logs** (`worker.log`, `logs/run.log`), chain checkpoints, posterior JSON — go to `/arc/projects/KILOGAS/analysis/toby_sandbox/kinuv_runs/{KGASID}-{YYYYMMDDTHHMMSSZ}-{kind}/`, never `$HOME`. `{KGASID}-latest` is a symlink to the newest run. `/scratch` is ephemeral: tee worker stdout to both scratch and `/arc` (line-buffered + fsync) so logs remain on `/arc` after fail, OOM, or success. Platform `canfar logs` expire in ~1 hour and vanish on 404; a submit-host watcher also copies `canfar logs`/`info`/`events` onto that run dir until the session is gone.
