...
2. Flexible resources by default: omit `--cpu` and `--memory`. Pass `--gpu` only when the live JAX build actually sees CUDA (this repo's recovery venv is CPU jax 0.11.1 / jax-finufft; do not request a GPU that that venv cannot use).
3. Fast compile / TMP: `/scratch/kinuv-$USER`. Manifests, logs, and posterior JSON: `/arc/home/thbrown/kinuv_runs/<run_id>/`.
