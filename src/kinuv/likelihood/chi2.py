"""Diagonal χ² with empirical weight scale ``s``.

``χ² = Σ s w |d − m|²`` is accumulated in float64. The blank
baseline uses zero complex visibilities. It is distinct from a non-rotating
emitting disk, which must be generated by the forward model and supplied as a
normal model array.
``s = 2 / ⟨w|V|²⟩`` on line-free fit channels. Not YAML 0.5. Not ``12/29``.
Reduced χ² is not a gate. Report blank and fitted non-rotating
improvements under their explicit names.
"""
...
def chi2_zero(vis, weights, s) -> float:
    """Compatibility name for :func:`chi2_blank`.

    Historical products called this the ``V=0`` score. It is actually the
    parameter-free, zero-complex-visibility baseline and contains no emitting
    disk. New validation code must use the explicit name ``chi2_blank``.
    """
    return chi2_blank(vis, weights, s)


@requires("DEC-066-WEIGHT", "DEC-066-ZEROMODEL")
def chi2_blank(vis, weights, s) -> float:
    """Return χ² for the parameter-free blank visibility model ``m=0``."""
    return chi2(vis, np.zeros_like(np.asarray(vis)), weights, s)


@requires("DEC-066-WEIGHT", "DEC-066-ZEROMODEL")
def chi2_nonrot(vis, nonrot_model, weights, s) -> float:
    """Return χ² for a fitted non-rotating *emitting* disk model.

    This function intentionally requires explicit model visibilities. Their
    brightness and nuisance parameters must be fitted or integrated with the
    same treatment used for the rotating model.
    """
    return chi2(vis, nonrot_model, weights, s)


@requires("DEC-066-ZEROMODEL")
def delta_chi2(chi2_val: float, chi2_zero_val: float) -> float:
    """Compatibility subtraction ``reference χ² - candidate χ²``."""
    return float(chi2_zero_val) - float(chi2_val)


def delta_chi2_blank(chi2_model: float, chi2_blank_val: float) -> float:
    """Improvement of an emitting model over blank complex visibilities."""
    return float(chi2_blank_val) - float(chi2_model)


def delta_chi2_nonrot(chi2_rot: float, chi2_nonrot_val: float) -> float:
    """Prior-free likelihood improvement of a rotating over non-rotating disk."""
    return float(chi2_nonrot_val) - float(chi2_rot)
