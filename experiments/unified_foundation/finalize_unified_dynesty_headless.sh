...
code_commit="${5:?sampler code commit required}"
finalizer_commit="${6:?finalizer code commit required}"
output="${7:?immutable output required}"
...
git -C "${workspace}/kinUV" archive "${finalizer_commit}" | tar -x -C "${repo}"
...
/arc/home/thbrown/kinuv-venv-recovery/bin/python \
  ...
  --map-commit "${map_commit}" --code-commit "${code_commit}" \
  --finalizer-commit "${finalizer_commit}" --output "${output}" \
  >"${log}" 2>&1
code=$?
set -e
...
printf '{"target":"%s","session_id":"%s","code_commit":"%s","finalizer_commit":"%s","exit_code":%d}\n' \
  "${target}" "${session_id}" "${code_commit}" "${finalizer_commit}" "${code}" \
