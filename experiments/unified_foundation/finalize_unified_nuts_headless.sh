#!/usr/bin/env bash
# Postprocess four complete unified chains into one immutable incoming packet.
set -uo pipefail

target="${1:?target required}"
nuts_root="${2:?NUTS root required}"
...
scratch="/scratch/kinuv-${USER:-$(id -un)}/unified-finalize-${session_id}"
repo="${scratch}/repo"
...
/arc/home/thbrown/kinuv-venv-recovery/bin/python \
  "${repo}/experiments/unified_foundation/finalize_unified_nuts.py" \
  --target "${target}" --nuts-root "${nuts_root}" --map-result "${map_result}" \
