"""Grep gate: the production record does not present KinMS as the vis fit."""

from pathlib import Path

REPO = Path(__file__).resolve().parents[1]
NOTES = REPO / "docs/PRODUCTION_RECORD.md"
DIRTY = (
    REPO
...
def test_notes_rank_and_not_kinms_posterior():
    ...
    assert text.lstrip().startswith("# kinUV production record")
    assert "binding model choices remain" in text
    assert "KinMS only as an image-plane comparator" in text
    assert "| KinMS |" not in text
    assert "|KinMS|" not in text
    lower = text.lower()
    assert "inflow" not in lower
    assert "\\dot{m}" not in lower
    assert "do not replace the visibility likelihood" in text
